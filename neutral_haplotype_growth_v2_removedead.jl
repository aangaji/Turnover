...
function neutral_growth!(tumor::Vector{Haplotype}, obs::Matrix, endsize::Int, T=Inf; b, d, μ, t=0., Nthresh=endsize, Tthresh = T, showprogress=true )
    ...
    while 0 < N < endsize && t < T
        ...
        if p > d
            ...
            for n=1:mnew
                ...
                obstemp[2] = isempty(parent.mutations) ? N : sum( getfield.(filter(h-> last(parent.mutations) in h.mutations, tumor[row : end-n]),:n) )
                obstemp[3] = N
            ...
            if mnew == 0
                ...
                F[row:end] .+= 1
            elseif mnew == 2
                if isone(parent.n) && !isone(row)
                    deleteat!(tumor, row)
                    deleteat!(F, row)
                    deleteat!(obsarr, row-1)
                else
                    parent.n -= 1
                end
                F[row:end] .-= 1
    ...
    obs = Matrix{Int}(undef, length(obsarr), size(obs,2))
    for i=1:size(obs,2)
